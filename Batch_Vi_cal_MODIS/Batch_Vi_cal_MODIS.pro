    ...
    'execute': BEGIN
     ...
     ;获取各景影像所在子文件夹
      ...
      FOR i=0,num-1 DO BEGIN
        ...
        ;建立存储数组
        NDVI=MAKE_ARRAY(2400,2400,46,/float)
        EVI=MAKE_ARRAY(2400,2400,46,/float)
        LSWI=MAKE_ARRAY(2400,2400,46,/float)
        ;开始计算
        FOR k=0, 45 DO BEGIN
        ...
        outfile = ofname +'ndvi'+ $
          PATH_SEP()+ crname + '_'+year+'_ndvi.img'
        ENVI_WRITE_ENVI_FILE, ndvi, OUT_NAME= outfile, BNAMES = bnames,$
          nb=46,r_fid=xfid,map_info=map_info
        outfile = ofname +'evi'+ $
          PATH_SEP()+ crname + '_'+year+ '_evi.img'
        ENVI_WRITE_ENVI_FILE, evi, OUT_NAME= outfile, BNAMES = bnames,$
          nb=46,r_fid=xfid,map_info=map_info
        outfile = ofname + 'lswi'+ $
          PATH_SEP()+ crname + '_'+year+'_lswi.img'
